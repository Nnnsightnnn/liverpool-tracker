...
export const PLAYER_EVIDENCE = {
  ...
  11: "Doubt for Monaco · still not training",   // Jacquet
...
export const SLOT_RATIONALE = {
  ...
  RCB: "Sat Aug 8 — the non-negotiable, and the club going quiet on it. Sky's reporting has cooled the Illia Zabarnyi talk, playing down a move for the Ukraine centre-back Iraola coached at Bournemouth and suggesting Liverpool do not plan a new defender. Palace's long-chased Marc Guehi keeps the file open with Milan's Tomori the cheaper route, but the nearer worry is fitness: Jacquet, twenty, has still not trained and is a doubt on the eve of Monaco, leaving Van Dijk, eased back through the friendlies, and the academy's Ndukwe. A body still has to arrive before Newcastle. This is the slot the whole window turns on.",
...
export const PREDICTION_NOTE = {
  level: "Low",
  reason: "Sat Aug 8 — the XI below stays a squad baseline rather than a matchday team, with the tour over and Monaco visiting Anfield on Sunday for Iraola's first home dugout. The defence frames the caution: Joe Gomez is out into September and misses the Newcastle opener, Leoni (ACL) is still recovering, and Jacquet, the only senior partner for Van Dijk, has still not trained and is a doubt on the eve of Monaco, the captain himself eased in through the friendlies. At right-back Frimpong, withdrawn against Leeds with what Iraola called overload, is effectively the only fit senior option, with Tottenham's Djed Spence lingering as a target. Off the pitch the marquee move nears its answer: reporting has Liverpool and PSG closing on the frame of a Barcola deal near €128m, cooled by Romano's caution that nothing is signed, while Curtis Jones has set his sights on Inter and Tottenham ready a fresh push for Gakpo. Salah was unveiled at Trabzonspor last week. Next up: Monaco at Anfield on August 9."
};
