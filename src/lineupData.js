...
export const FORMATIONS = {
  "4-3-3": {
    ...
    // Curtis Jones' €35m sale to Inter is confirmed so he leaves the squad. Gakpo, whose
    // £72m Tottenham exit is now reported days away, still starts on the right until a
    // winger lands. On the ledger the Barcola deal is a medical from here we go (agreed
    // in excess of £110m, unsigned); Newcastle's final Fernandez-Pardo bid was rejected.
    defaultXI: {
      ...
      RW: 22,   // Gakpo (£72m Spurs exit days away, but starts until a winger signs)
    },
  },
  ...
  "4-2-3-1": {
    ...
    // squad. Gakpo, £72m Tottenham move reported days away, is still in line to start
    // until a winger arrives; the Barcola deal is a medical from here we go (unsigned).
    defaultXI: {
      GK: 1,    // Alisson (started the 2-2 at Newcastle · No. 1)
      LB: 7,    // Kerkez (first-pick LB with Robertson gone to Spurs)
      LCB: 3,   // Van Dijk (captain · contracted to 2027)
      ...
      RAM: 22,  // Gakpo (starts on the right until a winger signs; £72m Spurs exit looms)
...
// Hand-curated from RESULTS array + injuryNote context as of Thu Aug 27.
...
// Inter is confirmed and he has left the squad; Gakpo's £72m Tottenham exit is
// reported days away but he is in line to start; the Barcola deal is a medical
// from here we go (unsigned); Newcastle's final Fernandez-Pardo bid was rejected.
...
export const PLAYER_EVIDENCE = {
  ...
  22: "Scored at Newcastle · £72m Spurs move days away", // Gakpo
  23: "Hostile return, no goal at St James'",        // Isak
  24: "Achilles · out until December",               // Ekitike
  25: "New 5-year deal to follow · rotation option", // Ngumoha
  26: "Touch and go vs Forest · no manager update",  // Chiesa
...
// (Refreshed Thu Aug 27: season underway (Newcastle away drawn 2-2 on Aug 23),
// next up Nottingham Forest at home on Aug 29, 12.30pm. Jones' €35m sale to Inter
// confirmed; Gakpo's £72m Spurs exit reported days away but he is in line to start;
// the Barcola deal is a medical from here we go (unsigned); Araujo in on loan;
// Frimpong the lone fit senior RB with Gomez out ~a month.)
export const SLOT_CONFIDENCE = {
  ...
  RAM: "Medium", // Salah gone (Trabzonspor) · Gakpo holds the right until a senior winger signs; £72m Spurs exit looms
...
export const SLOT_RATIONALE = {
  ...
  LAM: "Thu Aug 27 — a second season, and finally a settled position. Wirtz shared barely 117 minutes with Isak and Ekitike across an injury-disrupted first year at £115m, but he was the best player on the pitch against Monaco, ran the attacks through him, and starts behind the striker in Iraola's shape. With the wide market finally moving (Barcola a medical from here we go), the creative burden he has carried alone may soon be shared.",
  ...
  RAM: "Thu Aug 27 — the slot that defined the summer, and the summer is finally moving. Fabrizio Romano frames the Barcola agreement as done in both clubs' eyes, a medical from here we go, which would at last give this position its senior owner; until he lands, Gakpo holds the right, having dragged Liverpool level at Newcastle in the 55th, even as his own £72m move to Tottenham is reported days away and reportedly hinges on Barcola arriving first. Newcastle's final bid for Lille's Fernandez-Pardo, the fallback, was rejected overnight. Rio Ngumoha, the rotation option, may yet need to be more than that.",
...
export const PREDICTION_NOTE = {
  level: "High",
  reason: "Thu Aug 27 — the XI below points at Nottingham Forest at home on Saturday, August 29, a 12.30pm kick-off and Iraola's first competitive match at Anfield, and confidence stays high because there is a competitive team sheet to read: ten of the eleven shown here started the 2-2 at Newcastle, Szoboszlai rescuing a debut point from a stoppage-time penalty and Gakpo taking the equaliser. Expect continuity. The defence stays thin: Iraola has ruled Gomez out roughly a month with a muscle injury, while Leoni (ACL, eyeing September), Bradley (knee), Ekitike (Achilles, out to December) and Jaros are definitively out, so Frimpong holds the right with Araujo's loan the in-game cover and Jacquet beside Van Dijk. Chiesa stayed touch and go with no fresh word from the manager. Isak leads the line seeking a first Liverpool goal at Anfield after a hostile, blank afternoon on Tyneside. On the ledger the window turned on Thursday: Romano has the Barcola agreement a medical from here we go, Gakpo's £72m Tottenham exit days away as the funding sale, and Newcastle's final Fernandez-Pardo bid rejected. Bajcetic is done bar the signature on a Celta Vigo return. Araujo, a loan arrival, is not yet PLAYERS-keyed, so Jacquet fills RCB in the graphic. Next up: Nottingham Forest at home, August 29, 12.30pm."
};
